...
class getPV(): #really need a class? change to a function?

	def __init__(self, pvName):
		self.pvName = pvName
	
    def getCurrentValue(self,mean, standard_dev):
        # Random sensor data for test
...
class liveViewer(QtWidgets.QMainWindow):
    ...
    def __init__(self, liveUpdateInterval=1):
        super(liveViewer,self).__init__()

        # How frequently to get sensor data and update graph
        self.liveUpdateInterval = liveUpdateInterval

        # Init sensor object which gives live data
        self.targetPV = getPV()

        # Init with default values
        self.current_position = self.targetPV.getCurrentValue(mean=0.0, standard_dev=0.1)
        ...
        self.log_size = 1 * 60 * 1000 / self.liveUpdateInterval
        ...
        self.targetPV_value = PyQt5.QtWidgets.QDoubleSpinBox()
        self.verticalLayout.addWidget(self.targetPV_value)
        self.targetPV_value.setDecimals(6)
        ...
        self.signalComm.position_updated.connect(self.targetPV_value.setValue)
        ...
        self.position_update_thread = Thread(target=self.read_position,
                                             args=(self.targetPV, self.liveUpdateInterval))
    ...
    def read_position(self, sensor_obj, update_interval):
        ...
        sc = SignalCommunicate() #don't understand why it's defined again, cant use line 85 defined? Maybe the thread
        sc.got_new_sensor_data.connect(self.handleLiveData)

        while True:
            # Get data and timestamp from sensor
            new_pos = sensor_obj.getCurrentValue(mean=0.0, standard_dev=0.1)
            ...
            QtTest.QTest.qWait(update_interval)

    def handleLiveData(self, new_pos, new_pos_time):
        print('Thread ={}          Function = handleLiveData()'.format(threading.currentThread().getName()))
        ...
        self.log_time.append(self.current_position_timestamp) #replace qith deque
        if len(self.log_time) > self.log_size:
            # Append new data to the log and remove old data to maintain desired log size
            self.log_time.pop(0)

        self.log_position_raw.append(self.current_position)#replace qith deque
        if len(self.log_position_raw) > self.log_size:
            # Append new data to the log and remove old data to maintain desired log size
            self.log_position_raw.pop(0)

        # Request a graph update
        # self.update_graph()                     # Uncomment this if you want update_graph() to run in the same thread as handleLiveData() function
