...
class xrf_3ID(QtWidgets.QMainWindow):
    ...
    def create_xanes_macro(self):
        ...
        return     build_xanes_map(first_sid, last_sid, wd = cwd,xrf_subdir = cwd, xrf_fitting_param_fln=param,
    ...
    def hxn_auto_loader(self, wd,param_file_name,scaler_name, buffer_time = 10):
        ...
        while True:
            ...
            while int(caget('XF:03IDC-ES{Sclr:2}_cts1.B')) < 5000:
                ...
                print('beam is not available: waiting for shutter to open')
                time.sleep(60)
        
            if (caget('XF:03IDC-ES{Status}ScanRunning-I')==0 and int(caget('XF:03IDC-ES{Status}ScanID-I') != sid)):        
                
                try:         
                    sid = int(caget('XF:03IDC-ES{Status}ScanID-I'))
                    self.pte_status.setPlainText(f'scan:{sid}')
                    print(f'calling scan {sid} from data broaker')
                    hdr = db[(sid)]
                
                except ValueError:
                    print('scan maybe incomplete; skipped')
                    pass
        
        
                if bool(hdr.stop) == True:
                    try:
                        print(f'{sid} collection completed')
                        make_hdf(sid, wd = wd, file_overwrite_existing=True)
                        print(f'{sid} h5 created')
                        #pyxrf_batch(sid,sid, wd = wd, param_file_name = param_file_name, scaler_name = scaler_name )
                        #print(f'{sid} fitted with pyxrf')
                        #print(f'idle for {buffer_time}seconds')
    ...
    def start_auto(self):
        ...
        self.hxn_auto_loader(wd = cwd,param_file_name = param,scaler_name=norm, buffer_time = self.sb_buffer_time.value())
